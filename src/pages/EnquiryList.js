  ...
  const allEnquery = async () => {
    ...
    try {
      if (localStorage.getItem("_userType") === "Buyer") {
        const response = await API.buyer_enqueris_id(
        ...
        const response = await API.seller_enqueris_id(
          localStorage.getItem("__userId"),
          header
        );
              ...
              <>
                {allEnqris.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      {item.isActive === "1" ? (
                        <Link to="/message" className="btn btn-info me-2">
                    ...
                    <td>
                      {item.isActive === "1" ? (
                        ...
                        <button
                          ...
                          onClick={() => sellerAccept(item.enquiryId)}
                        >
                          {" "}
                          Accept
                        </button>
